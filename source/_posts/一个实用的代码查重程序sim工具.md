...
2. 修改`BINDIR`的值，推荐设置为`/usr/bin/`，一般来说，放到$PATH下比较方便。
3. 修改`MAN1DIR`的值，推荐设置为`/usr/share/man/man1`，这个目录是系统man文档的默认目录。
4. 将`sim.1`文件放到刚才设置的`MAN1DIR`里，放进去之后，`man sim`命令就应该可用了。
5. 安装依赖，一般来说，安装了`gcc`,`make`,`flex`这几个东西就够了。
6. 执行`make install`。
